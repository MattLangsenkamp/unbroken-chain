...
import ubc.githubgateway.domain.GhInstallationId
import zio.*
import zio.test.*

object TapirGitHubGatewayClientSpec extends ZIOSpecDefault:
  ...
  override def spec = suite("TapirGitHubGatewayClientSpec")(
    test("initiate() parses the response into LinkInitiation") {
      ...
      val client = new TapirGitHubGatewayClient(stubFor(responseJson), baseUri)
    ...
    test("unlink(...) returns Unit on 200, fails on 4xx") {
      val okClient   = new TapirGitHubGatewayClient(stubFor(""), baseUri)
      val errorBody  = """{"code":"GITHUB_FAILURE","message":"boom"}"""
      val failClient = new TapirGitHubGatewayClient(stubStatus(StatusCode.BadGateway, errorBody), baseUri)
      for
        ok  <- okClient.unlink(GhInstallationId(42L)).either
        bad <- failClient.unlink(GhInstallationId(42L)).either
      yield assertTrue(ok.isRight, bad.isLeft)
    },
    test("health() succeeds against an empty 200 body") {
      val client = new TapirGitHubGatewayClient(stubFor(""), baseUri)
