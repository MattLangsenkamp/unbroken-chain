...
/** Local DB surrogate primary key for an installation row. We own this identifier, so it is a UUID generated app-side
  * (never a DB sequence) — see the `relational-database-modeling` skill.
  */
object InstallationId extends Newtype[UUID]
type InstallationId = InstallationId.Type

/** Sentinel "not yet persisted" installation id (all-zero UUID). [[ubc.githubgateway.core.ports.GitHubAppClient]]
  * returns it before the local row exists; the repository assigns the real id on upsert.
  */
val UnpersistedInstallationId: InstallationId = InstallationId(new UUID(0L, 0L))

/** GitHub's numeric `installation_id` (the value GitHub itself assigns and echoes back in webhooks and API responses).
  * A third-party identifier — we follow GitHub's API, which models it as an integer. Distinct from [[InstallationId]]
...
/** Local DB surrogate primary key for a linked_repo row. We own this identifier, so it is a UUID generated app-side.
  */
object RepositoryId extends Newtype[UUID]
type RepositoryId = RepositoryId.Type

/** GitHub's numeric `repository_id`. Third-party identifier — Long. Distinct from [[RepositoryId]] which is our local
  * surrogate.
  */
...
/** Server-generated nonce used to correlate a link initiation with its callback. We own this value, so it is a UUID
  * generated from our `SecureRandom` port.
  */
object LinkState extends Newtype[UUID]
type LinkState = LinkState.Type

/** Value of the `X-GitHub-Delivery` header — a UUID GitHub assigns to each webhook delivery. GitHub's API dictates a
  * UUID, so we model it as one.
  */
