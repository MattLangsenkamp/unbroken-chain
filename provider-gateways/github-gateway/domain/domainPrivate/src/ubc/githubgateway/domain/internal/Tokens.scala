...
/** Short-lived RS256 JWT signed with the GitHub App's private key. Used to mint installation tokens; never leaves this
  * service. Marked `Sensitive` so `ActivityEncoder` redacts it.
  */
...
/** Installation access token minted on demand by exchanging an [[AppJwt]] for a given installation. Never persisted;
  * never leaves this service. Marked `Sensitive` so `ActivityEncoder` redacts it.
  */
