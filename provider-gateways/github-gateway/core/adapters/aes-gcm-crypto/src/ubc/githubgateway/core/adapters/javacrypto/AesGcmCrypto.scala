...
final class AesGcmCrypto(
  ...
  def encrypt(plaintext: String): Task[EncryptedBytes] =
    ZIO.attempt {
      ...
      EncryptedBytes(java.util.Base64.getEncoder.encodeToString(combined))
    }

  def decrypt(ciphertext: EncryptedBytes): Task[String] =
    ZIO.attempt {
      val combined = java.util.Base64.getDecoder.decode(ciphertext.unwrap)
      ...
      new String(cipher.doFinal(raw), java.nio.charset.StandardCharsets.UTF_8)
    }
...
object AesGcmCrypto:
  ...
  /** Construct a Crypto adapter from a base64-encoded 256-bit AES key (32 bytes raw).
    ...
    */
  def fromKey(encryptionKey: EncryptionKey): Task[AesGcmCrypto] =
    ZIO.attempt {
      val raw = java.util.Base64.getDecoder.decode(encryptionKey.unwrap)
