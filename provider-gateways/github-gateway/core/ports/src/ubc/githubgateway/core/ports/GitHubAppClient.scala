...
/** Outbound port for calls to GitHub's REST API made *as the App* (not as a user).
  *
  * All operations require either an [[AppJwt]] (App-level auth) or an
  * [[InstallationAccessToken]] (installation-level auth) — the caller is responsible for
  * minting one with [[InstallationTokenMinter.mintAppJwt]] and (where needed) exchanging via
  * [[createInstallationToken]] before calling.
  */
trait GitHubAppClient:
  ...
  /** `GET /app/installations/{installation_id}` — fetch the installation as known to GitHub.
    ...
    * keeps the port narrow at the cost of one trivial code-smell that core service code must
    * handle.)
    */
  def getInstallation(jwt: AppJwt, ghInstallationId: GhInstallationId): Task[Installation]

  /** `GET /installation/{installation_id}/repositories` — fetch the selected-repo set.
    * Requires an installation token (NOT the App JWT).
    */
  ...
  /** `POST /app/installations/{installation_id}/access_tokens` — exchange an App JWT for a
    ...
    * core service is responsible for any caching it wants.
    */
