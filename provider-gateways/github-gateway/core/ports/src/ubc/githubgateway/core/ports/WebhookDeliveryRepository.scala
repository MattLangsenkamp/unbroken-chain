...
trait WebhookDeliveryRepository:
  ...
  /** Update the recorded `outcome` for an existing delivery row.
    *
    * Used after dispatch resolves the final outcome (Processed / Ignored / Failed). No-op if
    * no row matches the given `deliveryId` — callers should always have called
    * [[recordIfAbsent]] first.
    *
    * Deviation from Layer 2: this method was added in Layer 3 to support recording the
    * post-dispatch outcome. The Layer 2 port shape only exposed `recordIfAbsent`, which seeded
    * the row with a provisional `Processed` outcome.
    */
