...
/** Outbound port for symmetric encryption of values stored at rest.
  *
  * The port is intentionally generic: it operates on `String` (UTF-8 plaintext) and `String`
  * (base64-encoded ciphertext) so the trait stays reusable across services. Service-specific
  * newtype wrappers (e.g. a `Ciphertext` opaque type) belong at the call site, not here.
  *
  * Adapters are responsible for choosing the cipher + IV scheme. The encryption key is a
  * constructor argument on each adapter — keys never flow through port signatures.
  */
trait Crypto:
