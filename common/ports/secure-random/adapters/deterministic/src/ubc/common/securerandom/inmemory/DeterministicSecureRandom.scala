...
/** Counter-backed deterministic [[SecureRandom]] for tests.
  *
  * Each call returns a string derived from a monotonically increasing counter, padded with `'A'` (URL-safe per RFC
  * 3986) to satisfy the requested length. Output is the exact string length of `urlSafeRandomString(byteCount)` from a
  * real adapter (`ceil(byteCount * 4 / 3)`) so length-sensitive code paths see the same shape.
  *
...
final class DeterministicSecureRandom(counter: Ref[Long]) extends SecureRandom:
  ...
  /** Deterministic UUID derived from the shared counter: the n-th call returns `00000000-0000-0000-0000-00000000000n`.
    * Predictable for assertions; never secure.
    */
